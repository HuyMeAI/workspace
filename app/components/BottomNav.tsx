...
export default function BottomNav({ isDarkMode, toggleTheme, onOpenModal, onShowToast }: any) {
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-[#09090b]/90 backdrop-blur-xl border-t border-zinc-200 dark:border-white/10 flex justify-around items-center h-16 pb-safe z-40 transition-colors duration-500 shadow-[0_-4px_24px_rgba(0,0,0,0.04)] dark:shadow-none">
       {/* SỬA <a> THÀNH <Link> VÀ SỬA href="#" THÀNH href="/" */}
       <Link href="/" className="flex flex-col items-center p-2 text-zinc-400 dark:text-zinc-500"><Home size={24} /></Link>
       <Link href="/folders" className="flex flex-col items-center p-2 text-zinc-400 dark:text-zinc-500"><Folder size={24} /></Link>
       ...
       <Link href="/schedule" className="flex flex-col items-center p-2 text-zinc-400 dark:text-zinc-500"><Calendar size={24} /></Link>
