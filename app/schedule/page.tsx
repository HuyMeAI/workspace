...
import db from '@/app/db/workspaceDB'; // SỬA ĐƯỜNG DẪN IMPORT CHUẨN
import { syncService } from '@/app/services/syncService'; // SỬA ĐƯỜNG DẪN IMPORT CHUẨN
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, X } from 'lucide-react';
import EditTaskModal from '@/app/components/EditTaskModal'; // SỬA ĐƯỜNG DẪN IMPORT CHUẨN

// Định nghĩa interface để triệt tiêu lỗi gạch chân 'any' của Strict Mode
...
export default function SchedulePage() {
  ...
  const TaskRow = ({ task }: { task: ScheduleTask }) => (
    <div 
      onClick={(e) => {
          e.stopPropagation(); 
          if(task.tag === 'Ngày lễ') {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 flex flex-col gap-1 w-full overflow-hidden">
                  {displayTasks.map((task: ScheduleTask) => <TaskRow key={task.id} task={task} />)}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm sm:p-4" onClick={() => setSelectedDailyDate(null)}>
          <div className="bg-white dark:bg-[#18181b] w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden relative" onClick={e=>e.stopPropagation()}>
            ...
            <div className="p-5 overflow-y-auto custom-scrollbar flex-1 space-y-3 pb-24 md:pb-5">
              {getTasksForDay(selectedDailyDate.getFullYear(), selectedDailyDate.getMonth(), selectedDailyDate.getDate()).map((task: ScheduleTask) => (
